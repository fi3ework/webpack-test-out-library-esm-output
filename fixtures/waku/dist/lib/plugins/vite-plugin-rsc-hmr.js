    ...
    return {
        ...
        async transform (code, id) {
            if (id.startsWith(wakuClientDist)) {
                ...
                const FETCH_RSC_LINE = 'export const fetchRSC = (input, searchParamsString, setElements, cache = fetchCache)=>{';
                return code.replace(FETCH_RSC_LINE, FETCH_RSC_LINE + `
{
  const refetchRsc = () => {
    cache.splice(0);
...
`);
                ...
                return code.replace(INNER_ROUTER_LINE, INNER_ROUTER_LINE + `
{
  const refetchRoute = () => {
