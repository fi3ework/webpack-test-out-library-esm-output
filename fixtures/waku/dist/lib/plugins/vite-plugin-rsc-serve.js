    ...
    return {
        ...
        config (viteConfig) {
            ...
            viteConfig.define = {
                ...
                'import.meta.env.WAKU_CONFIG_PUBLIC_DIR': JSON.stringify(opts.publicDir),
                'import.meta.env.WAKU_CONFIG_INDEX_HTML': JSON.stringify(opts.indexHtml)
            };
            if (opts.serve === 'cloudflare') {
                ...
                if (Array.isArray(viteConfig.build.rollupOptions.external)) {
                    viteConfig.build.rollupOptions.external.push('hono', 'hono/cloudflare-workers', '__STATIC_CONTENT_MANIFEST');
                } else {
