...
const emitRscFiles = async (rootDir, config, distEntries, buildConfig)=>{
    ...
    await Promise.all(Array.from(buildConfig).map(async ({ entries, context })=>{
        for (const { input, isStatic } of entries || []){
            ...
            const destRscFile = joinPath(rootDir, config.distDir, config.publicDir, config.rscPath, encodeInput(input));
            ...
            await pipeline(Readable.fromWeb(readable), createWriteStream(destRscFile));
        }
    }));
    return {
        getClientModules
...
const pathname2pathSpec = (pathname)=>pathname.split('/').filter(Boolean).map((name)=>({
            type: 'literal',
            name
        }));
...
const emitHtmlFiles = async (rootDir, config, distEntriesFile, distEntries, buildConfig, getClientModules, ssr)=>{
    ...
    await Promise.all(Array.from(buildConfig).map(async ({ pathname, isStatic, entries, customCode, context })=>{
        ...
        const destHtmlFile = joinPath(rootDir, config.distDir, config.publicDir, extname(pathname) ? pathname : pathname + '/' + config.indexHtml);
        const htmlReadable = ssr && await renderHtml({
            config,
            pathname,
            searchParams: new URLSearchParams(),
            htmlHead,
        ...
        if (htmlReadable) {
            await pipeline(Readable.fromWeb(htmlReadable), createWriteStream(destHtmlFile));
        } else {
...
export async function build(options) {
    ...
    await emitHtmlFiles(rootDir, config, distEntriesFile, distEntries, buildConfig, getClientModules, !!options.ssr);
    if (options.deploy?.startsWith('vercel-')) {
        ...
        await emitNetlifyOutput(rootDir, config, options.deploy.slice('netlify-'.length));
